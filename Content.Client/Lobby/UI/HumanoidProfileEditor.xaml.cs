...
namespace Content.Client.Lobby.UI
{
    ...
    public sealed partial class HumanoidProfileEditor : BoxContainer
    {
            ...
            JobPrototype job)
        {
            ...
            if (MakeRoleTestButton(job) is { } roleTestButton)
                jobContainer.AddChild(roleTestButton);
        ...
        private Button? MakeRoleTestButton(JobPrototype job)
        {
            var requirement = GetRoleTestRequirement(job);
            if (requirement == null)
                return null;

            var passed = IsRoleTestPassed(requirement);
            var button = new Button
            {
                Text = passed
                    ? Loc.GetString("role-test-job-button-passed")
                    : Loc.GetString("role-test-job-button"),
                ToolTip = requirement.Name,
                Disabled = passed,
                HorizontalAlignment = HAlignment.Right,
                VerticalAlignment = VAlignment.Center,
                Margin = new Thickness(3f, 3f, 0f, 0f),
            ...
            button.OnPressed += _ =>
                _consoleHost.ExecuteCommand($"roletests {CommandParsing.Escape(requirement.Test)}");

            return button;
        }

        private RoleTestRequirement? GetRoleTestRequirement(JobPrototype job)
        {
            var requirements = _entManager.System<SharedRoleSystem>().GetJobRequirement(job);
            return requirements?.OfType<RoleTestRequirement>().FirstOrDefault();
        }

        private bool IsRoleTestPassed(RoleTestRequirement requirement)
        {
            var player = _playerManager.LocalSession;
            if (player == null)
                return false;

            var tracker = RoleTestShared.GetTracker(requirement.Test);
            return _requirements.GetPlayTimes(player).TryGetValue(tracker, out var passed) &&

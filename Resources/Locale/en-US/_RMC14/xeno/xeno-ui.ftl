rmc-xeno-words-of-the-queen-header = The words of the Queen reverberate in our head...

rmc-xeno-word-queen-window-title = Word of the Queen
rmc-xeno-word-queen-window-send = Send to the hive

xeno-ui-reserve-parasites-title = Reserve Children
xeno-ui-reserve-parasites-apply-button-text = Apply
xeno-reserve-parasites-verb = Reserve Children

xeno-ui-find-parasite-title = Find Parasite
xeno-ui-find-parasite-spawn-button = Take Parasite
xeno-ui-find-parasite-item = {$itemName} ({$areaName})
xeno-ui-default-area-name = Unknown

xeno-ui-name-tunnel-title = Name Tunnel
xeno-ui-name-tunnel-submit-text = Apply Name
xeno-ui-rename-tunnel-verb = Rename Tunnel
xeno-ui-enter-tunnel-verb = Enter Tunnel

xeno-ui-select-destination-tunnel-title = Select Destination Tunnel
xeno-ui-select-destination-tunnel-submit-text = Select Tunnel

rmc-xeno-ui-burrowed-need-time = You ghosted too recently. You cannot become a burrowed larva until 2.5 minutes have passed ({$seconds} seconds remaining).
rmc-xeno-larva-queue-title = Join as Xeno
rmc-xeno-larva-queue-hives = Available Hives
rmc-xeno-larva-queue-search = Search hives...
rmc-xeno-larva-queue-count = {$count} hives
rmc-xeno-larva-queue-empty = No hives available
rmc-xeno-larva-queue-no-results = No matching hives
rmc-xeno-larva-queue-status-available = Not queued
rmc-xeno-larva-queue-status-position = Position {$position}
rmc-xeno-larva-queue-status-waiting = Waiting for queue
rmc-xeno-larva-queue-join = Join Queue
rmc-xeno-larva-queue-leave = Leave Queue
rmc-xeno-larva-queue-join-option = Join Larva Queue ({$hive})
rmc-xeno-larva-queue-leave-option = Leave Larva Queue ({$hive}) - Position {$position}
rmc-xeno-larva-prequeue-leave-option = Leave Larva Pre-Queue ({$hive})
rmc-xeno-larva-queue-round-delay = Joining a hive while observing is disabled at round start ({$seconds} seconds remaining).
rmc-xeno-larva-queue-removed = You have been removed from the larva queue.
rmc-xeno-larva-queue-added = You have been added to the larva queue at position {$position}.
rmc-xeno-larva-prequeue-added = You have been added to the larva pre-queue ({$seconds} seconds remaining).
rmc-xeno-larva-queue-position = You are now in position {$position} for the larva queue.
rmc-xeno-larva-queue-confirm-title = Join as Xeno
rmc-xeno-larva-queue-confirm-message = You are about to become {$xeno}. Click here to confirm within {$seconds} seconds.
rmc-xeno-larva-queue-confirm-option = Click here to confirm
rmc-xeno-larva-queue-confirm-decline-option = Decline
rmc-xeno-larva-queue-confirm-timeout = You did not confirm in time and lost your queue spot.
rmc-xeno-larva-queue-confirm-declined = You declined and lost your queue spot.
rmc-xeno-larva-queue-confirm-invalid = That xeno is no longer available.
rmc-xeno-larva-queue-burrowed-larva = a burrowed larva

rmc-xeno-tracker-target-Queen = Queen
rmc-xeno-tracker-target-HiveLeader = Hive Leader
rmc-xeno-tracker-target-Tunnel = Tunnel
rmc-xeno-tracker-target-HiveCore = Hive Core

rmc-xeno-ui-ovi-needed-label = [bold][color=red]The Queen must be in their\novipositor for you to gain points![/color][/bold]
rmc-xeno-ui-evolution-points = Evolution points: { $points } / { $maxPoints }
rmc-ui-xeno-strain-confirm = Confirm
